...
public class Default implements SlashHandler {
    ...
    public void onSlashCommand(SlashCommandInteractionEvent event, EventContext context) {
        ...
        try {
            ...
            event.reply("Invalid Product. Please use the auto completion.")
                    .setEphemeral(true)
                    .queue();
            return;
        }
    ...
    private Optional<MenuEntry<?, ?>> getReleaseTypeMenu(Member member, Product product) {
        ...
        if (available.size() == 1) {
            var releases = product.downloads()
                    .byReleaseType(available.stream().findFirst().get());
    ...
    private Optional<MenuEntry<?, ?>> getVersionMenu(Member member, Product product, List<Download> download) {
        ...
        List<AssetXO> assets = download.stream()
                .map(Download::latestAssets)
                .flatMap(List::stream)
        ...
        assets.reversed().stream()
                .limit(25)
                .forEach(asset -> versionMenu.addOption(
    ...
    private Optional<MenuEntry<?, ?>> getDownloadMenu(Member member, List<AssetVersion> available) {
        StringSelectMenu.Builder downloadMenu =
                StringSelectMenu.create("download").setPlaceholder("Please choose a type");
        available.forEach(a -> downloadMenu.addOption(
                a.download.type().name(), a.asset.id(), a.download().type().description()));

        return Optional.of(MenuEntry.of(downloadMenu.build(), ctx -> {
            var id = ctx.event().getInteraction().getSelectedOptions().get(0).getValue();
            Optional<AssetVersion> first =
                    available.stream().filter(d -> d.asset.id().equals(id)).findFirst();
